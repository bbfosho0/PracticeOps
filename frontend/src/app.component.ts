...
const SPECTRUM_HEIGHTS = [42,54,64,48,72,76,59,44,68,79,61,50,70,64,47,55,78,82,65,52,58,76,71,46,54,69,84,62,48,73,88,66,52,80,74,56,49,72,83,63,51,68,79,58,45,65,76,61,49,70,85,66,53,74,81,60,47,67,78,55,44,64,73,57,49,69,82,62,51,71,77,59];
...
export class AppComponent {
  ...
  readonly noteQueue = computed<NoteQueueItem[]>(() => this.dashboard().notes.slice(0, 6).map((note, index) => ({
    id: note.id,
  ...
  readonly runwayBlocks = computed<RunwayBlock[]>(() => {
    ...
    const layout = [[1,'2 / span 2'],[1,'4 / span 2'],[1,'7 / span 2'],[2,'2 / span 2'],[2,'5 / span 2'],[2,'8 / span 2'],[3,'3 / span 2'],[3,'5 / span 2'],[3,'8 / span 2'],[4,'2 / span 2'],[4,'4 / span 2'],[4,'7 / span 2'],[5,'3 / span 2'],[5,'5 / span 2'],[5,'8 / span 2'],[6,'2 / span 2'],[6,'4 / span 2'],[6,'7 / span 2']] as const;
    return layout.map((placement, index) => {
      ...
      return { id: `${appointment?.id ?? 'runway'}-${index}`, patient: appointment?.patientDisplayName ?? 'Synthetic patient', clinician: appointment?.clinician ?? 'Clinical team', service: appointment?.service ?? 'Therapy session', status, time: `${8 + Math.floor(index / 3)}:${index % 2 === 0 ? '00' : '30'}`, row: placement[0], column: placement[1], tone: toneForStatus(status) };
    });
  });
  ...
  readonly payerWatchlist = computed(() => {
    ...
    return [...grouped.entries()].sort((a, b) => b[1] - a[1]).slice(0, 5).map(([payer, exposure], index) => ({ payer, exposure, severity: index < 2 ? 'High' : index < 4 ? 'Medium' : 'Low', tone: index < 2 ? 'coral' : index < 4 ? 'amber' : 'green' }));
  });
  ...
  readonly eventTypeMetrics = computed<EventTypeMetric[]>(() => {
    ...
    const seeds = [['Note signed',2451,'cyan'],['Claim created',2102,'violet'],['Claim status change',1886,'green'],['Documentation updated',1304,'blue'],['Outbox delivered',1129,'amber'],['User or role change',472,'violet'],['Integration error',98,'coral'],['Security or access',87,'coral']] as const;
    return seeds.map(([label, count, tone]) => ({ label, count, share: `${((count / (9842 + total)) * 100).toFixed(1)}%`, tone }));
  });
  ...
  selectView(view: ViewId): void { this.activeView.set(view); }
  selectScheduleFilter(filter: 'day' | 'week' | 'list'): void { this.selectedScheduleFilter.set(filter); }
  load(): void {
    this.loading.set(true); this.apiMode.set('connecting'); this.notice.set('');
    this.http.get<Dashboard>('/api/dashboard').subscribe({
      next: value => { this.dashboard.set(value); this.apiMode.set('live'); this.loading.set(false); },
      error: () => { this.dashboard.set(createDemoDashboard()); this.apiMode.set('demo'); this.notice.set('API offline. Showing the complete local synthetic dataset.'); this.loading.set(false); }
    });
  }
  statusLabel(value: string): string { return humanizeStatus(value); }
  tone(value: string): SignalTone { return toneForStatus(value); }
  initials(value: string): string { return initials(value); }
  ...
  progress(value: number): number { return clampPercent(value); }
  round(value: number): number { return Math.round(value); }
