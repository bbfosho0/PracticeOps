export type AtmosphereState = 'active' | 'fallback' | 'reduced' | 'paused';

export function resolveAtmosphereState(options: { webglAvailable: boolean; reducedMotion: boolean; hidden: boolean }): AtmosphereState {
  if (!options.webglAvailable) return 'fallback';
  if (options.reducedMotion) return 'reduced';
  return options.hidden ? 'paused' : 'active';
}

/** Lightweight decorative WebGL2 aurora with CSS as the reliable fallback. */
export class AtmosphereRenderer {
  ...
  start(): void {
    ...
    this.draw = now => {
      if (!this.gl || !time || !resolution) return;
      this.gl.viewport(0, 0, this.canvas.width, this.canvas.height);
      this.gl.uniform1f(time, (now - this.startedAt) / 1000);
      this.gl.uniform2f(resolution, this.canvas.width, this.canvas.height);
      this.gl.drawArrays(this.gl.TRIANGLES, 0, 3);
      if (this.isAnimating()) this.frame = requestAnimationFrame(this.draw!);
    };
    ...
    this.resizeObserver = new ResizeObserver(() => {
      this.resize();
      if (this.isAnimating() && !this.frame) this.frame = requestAnimationFrame(this.draw!);
    });
  ...
  private sync(): void {
    ...
    cancelAnimationFrame(this.frame);
    this.frame = 0;
    const state = resolveAtmosphereState({ webglAvailable: true, reducedMotion: this.reducedMotion.matches, hidden: document.hidden });
    this.setState(state);
    if (state === 'active') this.frame = requestAnimationFrame(this.draw);
  }

  private isAnimating(): boolean {
    return Boolean(this.gl && this.draw && !document.hidden && !this.reducedMotion.matches);
  }
  ...
  private setState(state: AtmosphereState): void {
