  ...
  template: `
    <div class="shell">
      <aside>
        ...
        <nav aria-label="PracticeOps workspace">
          @for (item of nav; track item) {
            <button type="button" [class.active]="activeView() === item" [attr.aria-current]="activeView() === item ? 'page' : null" (click)="selectView(item)">{{ item }}</button>
          }
        </nav>
        <p class="synthetic">Synthetic data only</p>
      </aside>
      <main>
        <header><div><span class="eyebrow">Live operations workspace</span><h1>{{ viewTitle() }}</h1><p>{{ viewDescription() }}</p></div><div class="avatar">YG</div></header>
        ...
        @if (dashboard(); as data) {
          @if (activeView() === 'Overview') { <section class="metrics">
          ...
          <section class="grid">
            @if (activeView() === 'Overview' || activeView() === 'Schedule') { <article class="panel wide"><h2>Today’s schedule</h2>
            ...
            @if (activeView() === 'Overview' || activeView() === 'Documentation') { <article class="panel"><h2>Documentation readiness</h2>
              @for (item of noteSummary(); track item.label) { <div class="summary"><strong>{{ item.value }}</strong><span>{{ item.label }}</span></div> }
            </article> }
            @if (activeView() === 'Overview' || activeView() === 'Claims') { <article class="panel wide"><h2>Claims risk queue</h2>
            ...
            @if (activeView() === 'Overview' || activeView() === 'Audit log') { <article class="panel"><h2>Recent activity</h2>
            ...
            @if (activeView() === 'Settings') { <article class="panel settings-panel"><h2>Demo preferences</h2><p>PracticeOps displays synthetic operational data only. Workspace settings are intentionally read-only in this portfolio demonstration.</p></article> }
          </section>
        }
      </main>
    </div>
...
export class AppComponent {
  ...
  readonly nav = ['Overview', 'Schedule', 'Documentation', 'Claims', 'Audit log', 'Settings'] as const;
  readonly activeView = signal<(typeof this.nav)[number]>('Overview');
  readonly viewTitle = computed(() => ({ Overview: 'Operations overview', Schedule: 'Today’s schedule', Documentation: 'Documentation readiness', Claims: 'Claims risk queue', 'Audit log': 'Audit activity', Settings: 'Workspace settings' })[this.activeView()]);
  readonly viewDescription = computed(() => ({ Overview: 'Monitor today’s schedule, documentation readiness, and revenue-cycle risk.', Schedule: 'Coordinate appointments and progression through the day.', Documentation: 'Prioritize draft and in-review clinical documentation.', Claims: 'Resolve validation, filing, and payer risk before revenue is delayed.', 'Audit log': 'Review immutable activity across the fictional demo workspace.', Settings: 'Review the boundaries of this synthetic-data demonstration.' })[this.activeView()]);
  ...
  selectView(view: (typeof this.nav)[number]): void { this.activeView.set(view); }
  load(): void {
